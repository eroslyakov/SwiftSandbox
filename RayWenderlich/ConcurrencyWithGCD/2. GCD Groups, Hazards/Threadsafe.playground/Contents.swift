...
func getQOS() -> String { "\(Thread.current.qualityOfService.rawValue) \(DispatchQueue.current!.qos.qosClass)" }
...
//    print("\(getQOS()) Current Name: \(nameChangingPerson.name)")
//  }
//}
//
//nameChangeGroup.notify(queue: DispatchQueue.global()) {
//  print("\(getQOS()) Final name: \(nameChangingPerson.name)")
//  PlaygroundPage.current.finishExecution()
//}
//print("\(getQOS()) Outside nameChangeGroup")
...
class ThreadSafePerson: Person {
  // TODO: Write this implementation
    let isolationQueue = DispatchQueue(label: "isolated.queue", attributes: .concurrent)
    
    override var name: String { isolationQueue.sync { super.name }}
