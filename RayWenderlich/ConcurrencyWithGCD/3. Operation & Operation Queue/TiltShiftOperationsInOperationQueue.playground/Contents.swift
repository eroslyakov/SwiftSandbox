...
for (id, image) in images.enumerated() {
    ...
    operation.completionBlock = {
        ...
        let appendOperation = BlockOperation {
            filteredImages.append(outputImage)
            print("filtered images count: \(filteredImages.count)")
        }
        if id == images.count - 1 {
            appendOperation.completionBlock = {
                print("All images are filtered and added")
                filteredImages
                PlaygroundPage.current.finishExecution()
            }
        }
        appendQueue.addOperation(appendOperation)
    }
